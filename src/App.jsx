...
function App() {
  ...
  const generateMnemonic = () => {
    ...
    setWalletAddress(wallet.address); // automatically derive address
  };
  ...
  const createWallet = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/create_wallet`, {
        address: walletAddress,
      });
      setBalance(res.data.balance_eth);
      alert("Wallet created successfully! Address: " + walletAddress);
    } catch (err) {
      console.error(err);
  ...
  const fetchBalance = async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/balance/${walletAddress}`);
      setBalance(res.data.balance_eth);
    } catch (err) {
      console.error(err);
  ...
  const fetchTransactions = async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/transactions/${walletAddress}`);
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {mnemonic && (
          <>
            ...
            <p><strong>Derived Address:</strong> {walletAddress}</p>
          </>
        )}
